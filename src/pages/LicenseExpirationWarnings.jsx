...
function buildWarnings(quals, drivers) {
  ...
  quals.forEach(q => {
    ...
    const docs = [
      { type: 'CDL', date: q.cdl_expiration_date },
      { type: 'Medical Card', date: q.medical_card_expiration_date },
    ];
    docs.forEach(({ type, date }) => {
      if (!date) return;
      const days = differenceInDays(parseISO(date), new Date());
      if (days <= 90) {
        warnings.push({ driver, qual: q, docType: type, expDate: date, days });
      }
    });
  });
...
function WarningGroup({ title, badgeClass, items, onSendReminder, sending }) {
      ...
      <div className="border border-border rounded-lg overflow-hidden">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-border">
            {items.map((w, i) => (
              <tr key={`${w.qual?.id}-${w.docType}-${i}`} className="hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3 text-right">
                  {w.driver?.email && (
                    <Button
                      ...
                      disabled={sending === `${w.qual?.id}-${w.docType}`}
                      onClick={() => onSendReminder(w)}
                    >
                      {sending === `${w.qual?.id}-${w.docType}`
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <WarningGroup
            ...
            badgeClass="text-orange-600 border-orange-300 bg-orange-50 dark:bg-orange-900/20"
            items={within30}
            onSendReminder={handleSendReminder}
            sending={sending}
          />
          <WarningGroup
            title="Expiring Within 31–90 Days"
