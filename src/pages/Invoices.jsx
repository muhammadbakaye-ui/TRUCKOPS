...
import { Search, Trash2, X, Check, Loader2, Copy } from 'lucide-react';
...
function InvoiceStatusSelect({ invoice, queryClient }) {
  ...
  return (
    <Select value={current} onValueChange={handleChange} disabled={saving}>
      <SelectTrigger className={`h-7 text-xs px-2 border rounded-md font-medium w-full ${INV_STATUS_STYLES[current] || ''}`}>
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
...
export default function Invoices() {
  ...
  const columns = [
    ...
    { header: 'Status', width: '100%', render: (r) => <div onClick={e => e.stopPropagation()} className="w-full"><InvoiceStatusSelect invoice={r} queryClient={queryClient} /></div> },
    {
      header: '', render: (r) => (
        <AlertDialog>
