...
<div class="page">
  ...
  <div class="top-header">
    <div class="company-block">
      <span class="co-name">${(company.company_name || 'Your Company').toUpperCase()}</span><br>
      ${company.address_1 ? `<span class="co-detail">${company.address_1}</span><br>` : ''}
      ${(company.city || company.state || company.zip) ? `<span class="co-detail">${[company.city, company.state, company.zip].filter(Boolean).join(', ')}</span><br>` : ''}
...
<div class="download-bar" style="position:fixed;bottom:24px;right:32px;display:flex;gap:10px;z-index:9999;">
  <button onclick="doPrint()" style="background:#166534;color:#fff;border:none;padding:10px 22px;font-size:13px;font-weight:bold;border-radius:6px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.25);">⬇ Download PDF</button>
</div>

<script>
var data = { load_number: '${invoice.load_number || ''}', ref_number: '${invoice.invoice_number || ''}' };
function doPrint() {
  setTimeout(function() {
    var filename = 'Load-' + (data.load_number || 'Invoice') + '-' + (data.ref_number || '') + '.pdf';
